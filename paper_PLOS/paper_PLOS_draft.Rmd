...
p.ml.tree.increasing.geneCounts.withSize <- ml.tree.increasing.geneCounts.withSize %>%
  ...
  scale_fill_viridis_c(limits=c(size.scale.min, size.scale.max), guide = guide_colorbar(title="Body size (ln(g))", title.position = "left", title.vjust=1,title.theme = element_text(face="bold", size=6))) +
  geom_tiplab(aes(label=Spc.short), offset = 0.4, color="black", size=2, parse=T)+
...
#Prep work
ml.tree.increasing.geneCounts.withSize.table <- ml.tree.increasing.geneCounts.withSize %>% 
  as_tibble %>% 
  mutate(joint.label = str_c(label, ": ", N.Genes))

ml.tree.increasing.geneCounts.withSize.table[which(ml.tree.increasing.geneCounts.withSize.table$label %in% genomeTable.atlantogenata$label),]$joint.label <- ""

ml.tree.increasing.geneCounts.withSize <- ml.tree.increasing.geneCounts.withSize.table %>% as.treedata()
...
p.ml.tree.increasing.geneCounts.withSize.withBL <- ml.tree.increasing.geneCounts.withSize %>% 
  ...
  geom_label_repel(aes(
    label=yes.me(lnSize, joint.label, threshold=7)
  ),
    size=3, direction = "x", nudge_x = -0.2, force=10, segment.color = "black",
    color="grey"
  ) + 
  geom_label_repel(aes(
    label=not.me(lnSize, joint.label, threshold=7)
  ),
    size=3, direction = "x",nudge_x = -0.05, force=10, 
    color="black") + 
  geom_tiplab(aes(label=not.me(lnSize, N.Genes, threshold=7)), geom = "label", size=3, color="black") + 
  geom_tiplab(aes(label=yes.me(lnSize, N.Genes, threshold=7)), geom = "label", size=3, color="grey") + 
  geom_nodelab2(aes(subset=(label=="Root"), label = "Atlantogenata", x=branch), geom="label",
               size=2,
               color="black")+
  theme_tree2() + 
  xlim(-0.01,0.4) +
  xlab("Rate of Change, Gene Copy Number") + 
  theme(
    legend.position="bottom",
    # panel.background = element_rect(fill = "lightgrey"),
    text = element_text(face = "plain", 
...
Among the major constraints on the evolution of large body sizes (and long life-spans) in animals is an increased risk of developing cancer. If all cells in all organisms have a similar risk of malignant transformation and equivalent cancer suppression mechanisms, organism with many cells should have a higher prevalence of cancer than organisms with fewer cells, particularly because large and small animals have similar cell sizes (@Savage2007). Consistent with this expectation there is a strong positive correlation between body size and cancer incidence within species, for example, cancer incidence increases with increasing adult height in humans [@Green2011; @Nunney:20181c2] and in dogs [@Dobson2013; @Dorn1968]. There is no correlation, however, between body size and cancer risk _between_ species; this lack of correlation is often referred to as ‘Peto’s Paradox’ [@Caulin2011; @Leroi2003; @Peto1975]. The ultimate resolution to Peto’s Paradox is obvious: large bodied and long-lived species evolved enhanced cancer protection mechanisms. However, identifiying the specific genetic, molecular, and cellular mechanisms that underlie the evolution of augmented cancer protection has been difficult [c.f. @Ashur-Fabian2004; @Seluanov2008; @Gorbunova2012; @Tian2013; @Sulak2015]. 

Among the challenges for discovering how animals evolved enhanced cancer protection mechanisms is identifying lineages in which large bodied species are nested within species with small body sizes. _Afrotherian_ mammals are generally small-bodied, but also include the largest extant land mammals. For example, maximum adult weights are ~70g in golden moles, ~120g in tenrecs, ~170g in elephant shrews, ~3kg in hyraxes, and 60kg in aardvarks [@HAGR]. While extant hyraxes are relatively small, the extinct Titanohyrax is estimated to have weighted up to ~1300kg [@Schwartz1995]. The largest members of _Afrotheria_, too, are dwarfed by the size of their recent ancestors: extant cows manatees are large bodied (~322-480kg) but are relatively small compared to the extinct Stellar’s sea cow which is estimated to have weight 8000-10000kg [@Scheffer1972]. Similarly African (4,800kg) and Asian elephants (3,200kg) are the largest living elephant species, but are dwarfed by the truly gigantic extinct Proboscideans such as Deinotherium (~132,000kg), Mammut borsoni (110,000kg), and the Asian straight-tusked elephant (~220,000kg), the largest known land mammal [@Larramendi:20151c2]. Remarkably, these large-bodied _Afrotherian_ lineages are nested within small bodied species (__Figure \@ref(fig:Figure-intro)__) [@OLeary2013a; @Springer2013; @OLeary2013b; @PuttickAndThomas2015], indicating that gigantism independently evolved in hyraxes, sea cows, and elephants (_Paenungulata_). Thus, Paenungulates are an excellent model system in which to explore the mechanisms that underlie the evolution of large body sizes and augmented cancer resistance. 

Although many mechanisms can potentially resolve Peto’s paradox, among the most parsimonious routes to enhanced cancer resistance is through an increased copy number of tumor suppressors. Indeed, candidate genes studies have found that the elephant genome encodes duplicate such as _TP53_ and _LIF_ [@Abegglen:JAMA2015; @Sulak2015; @Vazquez2018] as well as other genes with putative tumor suppressive functions [@Caulin2015; @Doherty2016]. As these studies focus on _a priori_ gene sets, however, it remains unknown whether this is a general, genome-wide trend in _Afrotherian_ genomes; and whether such a general trend is associated with the recent increases in body size – and therefore expected cancer risk – in these species. 

Here, we trace the evolution of body mass and gene copy number variation in across _Afrotherian_ genomes in order to investigate whether duplication of tumors suppresor is common in large, long-lived Proboscideans. Our estimates of the evolution of body mass, similarly to previous studies [@OLeary2013a; @Springer2013; @OLeary2013b; @PuttickAndThomas2015], show that large body masses evolved in a step-wise manner, with major increases in body mass in the _Pseudoungulata_ (17kg), _Paenungulata_ (25kg), _Tethytheria_ (296kg), and _Proboscidea_ (4,100kg) stem-lineages. To explore whether duplication of tumor suppressor genes occurred coincident with the evolution of large body sizes, we used a genome-wide Reciprocal Best BLAT Hit (RBBH) strategy to identify gene duplications, and used maximum likelihood to infer the lineages in which those duplications occurred. Unexpectedly, we found that duplication of tumor suppressor genes was common in all Afrotherians, both large and small. These data suggest that duplication of tumor suppressor genes is pervasive in Afrotherians and proceeded the evolution of species with very large body sizes.
...
ORA.TSG.list <- ORA.all %>% filter(Direction=="increased") %>% 
  ...
  ungroup %>% 
    select(Node, RBHB, Database, Pathway, Genes, FDR.Threshold) %>% 
    distinct %>% 
    mutate(
...
## Step-wise evolution of body size in _Afrotherians_
...
p.tree.cancerSucceptibility.Atlantogenata <- tree.cancerSucceptibility.Atlantogenata %>% as_tibble %>% filter(!is.na(node)) %>%  mutate(branch.length = log2CancerSucceptabilityChange %>% abs()) %>% as.treedata %>% 
  ggtree(
...
As expected, intrinsic cancer susceptibility in Afrotheria also varies with changes in body size and longevity (Figure \@ref(fig:Figure-CancerSuscept), Table \@ref(tab:Table-CancerSuscept)), with an initial `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Afrotheria") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold and `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Xenarthra") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increase in the stem-lineage of Afrotheria and Xenarthra, respectively, followed by a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Pseudoungulata") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increases in Pseudoungulata and a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Orycteropus afer") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increase in Aardvarks (Figure \@ref(fig:Figure-CancerSuscept), Table \@ref(tab:Table-CancerSuscept)). In contrast to the Paenungulate stem-lineage, there is a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Tethytheria") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increase in cancer risk in Tethytheria, a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Trichechus manatus") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increase in Manatee, and dramatic increases within Proboscidea including a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Elephantidae") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increase in Elephantidae and a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Mammut americanum") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold in the American Mastodon. Within the Elephantidae, Elephantina and Loxodontini have a `r table.cancerSucceptibility.Atlantogenata.genomes %>% filter(label == "Elephantina") %>% pull(log2CancerSucceptabilityChange) %>% round(2) %>% as.numeric`-fold increase in cancer susceptibility, while susceptibility is relatively stable in Mammoths. The three extant Proboscideans, Asian Elephant, African Savana Elephant, and the African Forest Elephant, meanwhile, have similar decreases in both size and cancer susceptibility (Figure \@ref(fig:Figure-CancerSuscept), Table \@ref(tab:Table-CancerSuscept)). 
...
<!-- We found that gene duplications were common in Atlantogenatan genomes (__Figure 4__), with number of duplications per lineage consistent with previously estimated gene duplication rates (REFS), but were particularly numerous in _Chrysochloris asiatica_,  _Elephantulus edwardii_. To explore whether genome quality may adversely effect our inferences, we used CEGMA and the gVolante server [@CEGMA; @gVolante] to assess the correlation between genome quality and copy number estimates. As shown in Supplementary Figure X, mean Copy Number, mean ECNC, and mean CN (the lesser of Copy Number and ECNC per gene) moderately or strongly correlate with genomic quality, such as LD50, the number of scaffolds, and contigs with a length above either 100K or 1M. -->

We found that gene duplications were common in Atlantogenatan genomes (Figure \@ref(fig:Figure-GeneDup-Cladogram)), identifying an average of `r genomeStatsRBHB$pc.found %>% mean %>% as.percentage` genes, with an average of `r genomeStatsRBHB$pc.Hits.Duplicated %>% mean %>% as.percentage()` duplicated; this is in-line with other studies describing the rates of gene duplications over time. [@Lynch2000]. We observed that the percentage of duplicated genes in non-_Pseudoungulatan_ genomes was significantly higher: while _Pseudoungulatan_ genomes had duplicates percentages ranging anywhere from `r genome.table.forFig %>% filter(!Species %in% c("Choloepus hoffmanni", "Chrysochloris asiatica", "Dasypus novemcinctus", "Echinops telfairi", "Elephantulus edwardii"))%>% pull("% Hits Duplicated") %>% str_remove("%") %>% as.numeric %>% divide_by(100) %>% min %>% as.percentage` to `r genome.table.forFig %>% filter(!Species %in% c("Choloepus hoffmanni", "Chrysochloris asiatica", "Dasypus novemcinctus", "Echinops telfairi", "Elephantulus edwardii")) %>%  pull("% Hits Duplicated") %>% str_remove("%") %>% as.numeric %>% divide_by(100) %>% max %>% as.percentage`, outgroup species' duplication rates ranged from `r genome.table.forFig %>% filter(Species %in% c("Choloepus hoffmanni", "Chrysochloris asiatica", "Dasypus novemcinctus", "Echinops telfairi", "Elephantulus edwardii"))%>% pull("% Hits Duplicated") %>% str_remove("%") %>% as.numeric %>% divide_by(100) %>% min %>% as.percentage` to `r genome.table.forFig %>% filter(Species %in% c("Choloepus hoffmanni", "Chrysochloris asiatica", "Dasypus novemcinctus", "Echinops telfairi", "Elephantulus edwardii")) %>%  pull("% Hits Duplicated") %>% str_remove("%") %>% as.numeric %>% divide_by(100) %>% max %>% as.percentage`. To explore whether genome quality may adversely effect our inferences, we used CEGMA and the gVolante server [@CEGMA; @gVolante] to assess the correlation between genome quality and copy number estimates. As shown in Figure \@ref(fig:Figure-GenomeCor), mean Copy Number, mean ECNC, and mean CN (the lesser of Copy Number and ECNC per gene) moderately or strongly correlate with genomic quality, such as LD50, the number of scaffolds, and contigs with a length above either 100K or 1M.
...
loxAfr4.geneTPM.ORA.expressedDups.TP53 <- loxAfr4.geneTPM.ORA.expressedDups %>% 
  ...
  ungroup %>% 
  filter(Reactome.Pathway.Prime!="", Database == "Reactome")
  
membership.matrix <- loxAfr4.geneTPM.ORA.expressedDups.TP53 %>% select(Gene, Reactome.Pathway.Prime)%>% distinct %>% pull(Reactome.Pathway.Prime) %>% str_split(", ") %>% set_names(., loxAfr4.geneTPM.ORA.expressedDups.TP53$Gene %>% unique)%>% binTableFromList() %>% data.table::transpose(make.names = "Gene", keep.names = "Gene")

loxAfr4.geneTPM.ORA.expressedDups.TP53 %<>% full_join(
  .,
  membership.matrix %>% select(Gene, membership.matrix %>% summarise_at(vars(-Gene), sum) %>% select_if(.>1) %>% names), 
  by="Gene"
  ) 
...
p.treedata.tp53.table <- treedata.tp53.table %>% 
  ...
  geom_point2(aes(x=branch, subset=(GTF2F1>0), color="GTF2F1"), size=1.5, position = position_nudge(x=-0.2, y=0.2)) +
  geom_point2(aes(x=branch, subset=(STK11>0), color="STK11"), size=1.5, position = position_nudge(x=0, y=0.2)) +
  geom_point2(aes(x=branch, subset=(COX5A>0), color="COX5A"), size=1.5, position = position_nudge(x=0.2, y=0.2)) +
  geom_point2(aes(x=branch, subset=(BRD7>0), color="BRD7"), size=1.5, position = position_nudge(x=-0.2, y=0)) +
  geom_point2(aes(x=branch, subset=(TP53>0), color="TP53"), size=1.5, position = position_nudge(x=0, y=0)) +
  # geom_point2(aes(x=branch, subset=(CASP9>0), color="CASP9"), size=1.5, position = position_nudge(x=0.2, y=0)) +
  geom_point2(aes(x=branch, subset=(PRDX1>0), color="PRDX1"), size=1.5, position = position_nudge(x=-0.2, y=-0.2)) +
  geom_point2(aes(x=branch, subset=(LAMTOR5>0), color="LAMTOR5"), size=1.5, position = position_nudge(x=0, y=-0.2)) +
  geom_point2(aes(x=branch, subset=(COX20>0), color="COX20"), size=1.5, position = position_nudge(x=0.2, y=-0.2)) +
  geom_tiplab(parse=T, offset = 0.25, size=2) +
...
(ref:Figure-CancerSuscept) Cancer succecptibility across _Atlantogenata._ Branch lengths are set to the magnitude of change in cancer succeptiblity; colors indicate the magnitude and direction of the change.

```{r Figure-CancerSuscept, fig.cap="(ref:Figure-CancerSuscept)", out.width="6in", fig.asp=1}
p.tree.cancerSucceptibility.Atlantogenata
```

(ref:Figure-GeneDup-Cladogram) Gene duplications occur readily throughout _Atlantogenata_. Shown here is a tree of _Atlantogenatan_ species with genomes, with the number of genes that underwent an increase in copy number overlayed at each node.

```{r Figure-GeneDup-Cladogram, fig.cap="(ref:Figure-GeneDup-Cladogram)", out.height="6in", out.width="6in"}
p.ml.tree.increasing.geneCounts.withSize
```

(ref:Figure-GenomeCor) Correlations between genome quality metrics and ECNC metrics. Gene copy number metrics, and the genome quality metrics most strongly associated with them, are highlighted in red.

```{r Figure-GenomeCor, fig.cap="(ref:Figure-GenomeCor)", fig.asp=1, out.width="6in", out.height="6in", fig.align="center"}
p.cor.GCT.CEGMA
```

(ref:Figure-loxAfrORA) Overrepresentation Analysis of Duplicated Genes in Atlantogenata using Reactome Pathways.

```{r Figure-loxAfrORA, fig.cap="(ref:Figure-loxAfrORA)", out.width="6in", out.height="4in"}
fig.loxAfr.ORA <- ggarrange(
  p.loxAfr.ORA.dotplot, 
  p.loxAfr.ORA.dotplot.increased, 
  nrow=1, ncol=2, 
...
(ref:Figure-loxAfrTP53) TP53-related genes are also duplicated and functional in _Loxodonta africana_. __A)__ Cladogram of _Atlantogenata_ highlighting along each branch when duplications for each gene occured. __B)__ Gene expression data collected from pulically-available RNA-seq data for each duplicate in __A)__.

```{r Figure-loxAfrTP53, fig.cap="(ref:Figure-loxAfrTP53)", out.width="6in"}
fig.TP53 <- ggarrange(
  p.treedata.tp53.table, 
  p.tp53.genes.expression, 
  nrow=1, ncol=2, 
  labels="AUTO", 
  widths = c(1.5,1)
  )
...
genomeTable.atlantogenata %>% 
  mutate(Citation="NA") %>% 
  select(Species, `Common Name`, Genomes = Genome, `Highest Quality Genome` = BestGenome, Citation) %>% 
  kable(
    ., 
    booktabs = TRUE,
...
```{r Figure-GeneDup-BLScaled, fig.cap="Gene copy increases polarized along Atlantogenata, colored by ln(Body Size), with branch lengths equal to the change in gene copy number", eval=T}
p.ml.tree.increasing.geneCounts.withSize.withBL
```

```{r Figure-RBHBStrategy, fig.cap="Full version of RecBlat strategy, but low quality", warning=T, eval=T}
image_read("../output/figs/RBHBDiagram.png") %>% 
  # image_rotate(180)%>% 
  image_flip() %>% 
  image_ggplot()
```
...
```{r InterestingGeneTrees, fig.cap="Supplementary Figure 5: All the Gene Copy Trees for interesting genes duplicated in LoxAfr4 (RIP any trees if this gets printed)", eval=F}
p.ml.treedata.loxAfrHighlight.plus
warning("Don't print the full list, seriously, its like 600+ pages")
```

```{r Figure-TimeTree-Eutheria, fig.cap="The time-calibrated Eutherian tree used as input for StableTraits, from Bininda-Emonds et al and Puttick and Thomas", warning=T, eval=T}
...
```{r Figure-TimeTree-Atlantogenata, fig.cap="The time-calibrated Atlantogenatan tree used as input for StableTraits, from Puttick and Thomas", warning=T, eval=F}
...
```{r Figure-DoubleTree-Bodysize-CancerSuscept-Genomes}
...
bsize.Atlantogenata.genomes <-  bsize.Atlantogenata@phylo %>% 
  treeio::drop.tip(bsize.Atlantogenata@phylo$tip.label[!bsize.Atlantogenata@phylo$tip.label %in% genomeTable.atlantogenata$label]) %>% as.tibble
...
### Cancer Suscept ###
tree.cancerSucceptibility.Atlantogenata.table <- tree.cancerSucceptibility.Atlantogenata %>% as_tibble
tree.cancerSucceptibility.Atlantogenata.table[which(tree.cancerSucceptibility.Atlantogenata.table$node == MRCA(tree.cancerSucceptibility.Atlantogenata, "Pseudoungulata", "Elephantulus edwardii")),]$label <- "Afroinsectivora"
tree.cancerSucceptibility.Atlantogenata <- tree.cancerSucceptibility.Atlantogenata.table %>% as.treedata()

cancerSucceptibility.Atlantogenata.genomes <- tree.cancerSucceptibility.Atlantogenata %>% 
  as_tibble() %>%  
...
tree.cancerSucceptibility.Atlantogenata.genomes <-  tree.cancerSucceptibility.Atlantogenata %>% 
  as_tibble %>% 
  filter(!is.na(node)) %>%  
  mutate(branch.length = log2CancerSucceptabilityChange %>% abs()) %>% 
  as.treedata %>% 
  as.phylo() %>% 
  treeio::drop.tip(tree.cancerSucceptibility.Atlantogenata@phylo$tip.label[!tree.cancerSucceptibility.Atlantogenata@phylo$tip.label %in% genomeTable.atlantogenata$Species.strict])  %>% as.tibble
tree.cancerSucceptibility.Atlantogenata.genomes <- left_join(tree.cancerSucceptibility.Atlantogenata.genomes, cancerSucceptibility.Atlantogenata.genomes) %>% as.treedata

p.tree.cancerSucceptibility.Atlantogenata.genomes <- tree.cancerSucceptibility.Atlantogenata.genomes %>% 
  ggtree(
...
d.dbltree.2 <- p.tree.cancerSucceptibility.Atlantogenata.genomes$data

d.dbltree.2$x <- max(d.dbltree.2$x) - d.dbltree.2$x + max(d.dbltree.1$x) + 500

p.dbltree <-
  bsize.Atlantogenata.genomes %>% 
  ggtree(., aes(color=lnSize), size=2) +
  scale_color_viridis_c(option = "A") + 
  geom_nodepoint(size=2) + 
  geom_tippoint(size=2) +
  geom_tiplab(x=(min(d.dbltree.2$x)+max(d.dbltree.1$x))/2, color="black", hjust = 0.5) +
  new_scale_color() + 
  geom_tree(data=d.dbltree.2, aes(color=log2CancerSucceptabilityChange), inherit.aes = F, size=2) + 
...
```{r Figure-DoubleTree-Bodysize-CancerSuscept-All}

### Body Size ###

p.tree.atlantogenata.genomes <- bsize.Atlantogenata %>% 
  ggtree(
    aes(color=as.numeric(lnSize))
  )
...
tree.cancerSucceptibility.Atlantogenata <-  tree.cancerSucceptibility.Atlantogenata %>% 
  as_tibble %>% 
  filter(!is.na(node)) %>%  
  mutate(branch.length = log2CancerSucceptabilityChange %>% abs()) %>% 
  as.treedata

p.tree.cancerSucceptibility.Atlantogenata.2 <- tree.cancerSucceptibility.Atlantogenata %>% 
  ggtree(
    aes(color=as.numeric(log2CancerSucceptabilityChange))
  )
...
d.dbltree.4 <- p.tree.cancerSucceptibility.Atlantogenata.2$data
...
p.dbltree.all <-
  ...
  geom_nodepoint(size=2) + 
  geom_tippoint(size=2) +
  geom_tiplab(x=(min(d.dbltree.4$x)+max(d.dbltree.3$x))/2, color="black", hjust = 0.5) +
  new_scale_color() + 
