...
knitr::opts_chunk$set(eval=T, echo=F, message = F, warning = F, # fig.width=6, 
                      fig.retina = 2, fig.align = "left", dpi = 300, dev="Cairo_pdf")
...
```
```{r Figure2, fig.cap="Cancer succecptibility across Atlantogenata"}
f2
```
...
```{r Figure4, fig.cap="Correlations between genome quality metrics and ECNC metrics"}
f4 <- p.cor.GCT.CEGMA
f4
```
...
```{r Figure5, fig.cap="Overrepresentation Analysis of Duplicated Genes in Atlantogenata using Reactome Pathways.", fig.width=6, fig.height=8}
...
```{r, Supplementary Figure 4, fig.cap="Supplementary Figure 4: Dot and Line plot for Body Sizes and such", eval=F}
...
```{r Supplementary Figure 7, fig.cap="Supplementary Figure 7: Correlation matrix heat map for genome quality metrics, ECNC, RBHB Copy, and Estimated Copy Number (lesser between ECNC and RBHB)", eval=F}
p.cor.GCT.CEGMA
```
