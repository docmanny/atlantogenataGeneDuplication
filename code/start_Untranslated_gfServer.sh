...
if $($gfServ status localhost $tPort &> /dev/null); then
    ...
    ($gfServ start localhost "$tPort" -canStop -stepSize=5 -repMatch=2253 "$BLATDB"/"$twoBitFile" &> /dev/null) &
...
touch "flags/untranslated-$spc-$tPort"; exit 0